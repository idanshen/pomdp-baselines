...
class MujocoManipulateTouchSensorsEnv(MujocoManipulateEnv):
    def __init__(
        ...
        **kwargs,
        ...
        for (k, v,) in (self._model_names.sensor_name2id.items()):  # get touch sensor site names and their ids
            if "robot0:TS_" in k:
                self._touch_sensor_id_site_id.append(
                    (v, self._model_names.site_name2id[k.replace("robot0:TS_", "robot0:T_")],)
                )
