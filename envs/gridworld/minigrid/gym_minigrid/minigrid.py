...
class WorldObj:
    ...
    def __init__(self, type, color, visible=True):
        assert type in OBJECT_TO_IDX, type
        assert color in COLOR_TO_IDX, color
        self.type = type
        self.color = color
        self.contains = None
        self.visible = visible  # Allows disabling of the rendering in the image.
...
class Goal(WorldObj):
    def __init__(self, visible=True):
        super().__init__('goal', 'green', visible)
...
class Floor(WorldObj):
    ...
    def render(self, r):
        ...
        c = COLORS[self.color]
        r.setLineColor(0. * 255.0, 0. * 255.0, 0.8 * 255.0)
        r.setColor(*c/2)
        r.drawPolygon([
            (1,             TILE_PIXELS),
...
class Lava(WorldObj):
    def __init__(self, visible=True):
        super().__init__('lava', 'red', visible)

    def can_overlap(self):
        return True

    def render(self, img):
        c = (0. * 255.0, 0. * 255.0, 0.8 * 255.0)
...
class Grid:
    ...
    def set(self, i, j, v):
        assert self.width > i >= 0
        assert self.height > j >= 0
    ...
    def render_tile(
        ...
        img = np.ones(shape=(tile_size * subdivs, tile_size * subdivs, 3), dtype=np.uint8) * np.asarray((0.84 * 255.0, 0.93 * 255.0, 0.94 * 255.0))
...
class MiniGridEnv(gym.Env):
    ...
    def __init__(
        ...
        # self.action_space.shape = (len(self.actions), )
        ...
        self.reward_range = (0.0, 20.0)
        assert self.reward_range[0] == 0  # Has to be 0.
        ...
        self.np_random = None                   # Hold the seed.  Not really used anymore.
        self.illuminated = False                # Start with the lights off.
        self.carrying = None                    # Item picked up, being carried, initially nothing
        self.step_count = 0                     # Step count since episode start
        self.seed(seed=seed)                    # Initialize the RNG
        self.full_rendering_obs_noise = 0.05    # Set the noise in renderings.
    ...
    def reset(self, _seed=None):
        ...
        self._gen_grid(self.width, self.height, _seed=_seed)
    ...
    def seed(self, seed=1337):
        ...
        if seed is not None:
            try:
                self.np_random, _ = seeding.np_random(int(np.rint(seed)))
            except:
                try:
                    self.np_random, _ = seeding.np_random(int(np.rint(seed + 0.000001)))
                except:
                    print("SEED ERROR: " + str(np.round(seed).astype(np.int)))
        else:
    ...
    def set_np_state(self, _state):
        return self.np_random.set_state(_state)
            ...
            for i in range(self.grid.width):
                ...
                if c.type == 'door':
                    if c.is_open:
                        ...
                        str_ += 'L' + c.color[0].upper()
                    else:
                        str_ += 'D' + c.color[0].upper()
                    continue

                str_ += object_to_str[c.type] + c.color[0].upper()
    ...
    def place_agent(
        ...
        # Use a random seed for generation.
        if _seed is None:
            _seed = SEED_FUNC()
        self.seed(_seed)
    ...
    def step(self, action):
        ...
        reward = -2.0  # A step costs something.
        done = False
    ...
    def special_render(self, mode='human', close=False, highlight=False, tile_size=TILE_PIXELS, render_goal=True, render_lava=True):
        """
