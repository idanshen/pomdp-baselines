...
def _my_render(env, env_name, tag='minigrid', resize=84, obs_type='observe'):
    ...
    if tag == 'minigrid':
        ...
        if (obs_type == 'observe') or (obs_type == 'full_observe'):
            ...
            observation = observation.copy().T
            ...
            observation = env.env.render(_key=obs_type)
        else:
...
class EnvWrapper(gym.Env):
    ...
    def __init__(self, env, params, add_t_with_horizon=None, observation_type=None):
        ...
        self.env = env
        self.params = params
        self._max_episode_steps = self.env.unwrapped._max_episode_steps
        ...
        if len(obs.shape) == 1:
            ...
            assert (obs.shape[0] == 3) or (obs.shape[0] == (3 * self.frame_stack))
            input_size = list(obs.shape)
            self.num_features = input_size
            self.observation_space = Box(np.zeros((self.num_features)), 255*np.ones((self.num_features)), self.num_features, np.uint8(1.).dtype)
    ...
    def my_render(self, flatten=None):
        ...
        stacked_obs = np.concatenate(self.frames, axis=0)       # get the stack.
