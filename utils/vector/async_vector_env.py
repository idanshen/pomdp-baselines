...
class AsyncVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        if self.shared_memory:
            try:
                ...
                _state_buffer = create_shared_memory(
                    self.single_state_space, n=self.num_envs, ctx=ctx
                )
                self.states = read_from_shared_memory(
                    _state_buffer, self.single_state_space, n=self.num_envs
                )
            ...
            self.states = create_empty_array(
    ...
    def reset_wait(self, timeout=None):
        ...
        observations, states, successes = zip(*[pipe.recv() for pipe in self.parent_pipes])
        self._raise_if_errors(successes)
        self._state = AsyncState.DEFAULT
        ...
        if not self.shared_memory:
            ...
            self.states = concatenate(
                states, self.states, self.single_state_space
            )

        return deepcopy(self.observations) if self.copy else self.observations, deepcopy(self.states) if self.copy else self.states
    ...
    def step_wait(self, timeout=None):
        ...
        if not self.shared_memory:
            ...
            self.states = concatenate(
                infos["state"], self.states, self.single_state_space
            )
        info = {"state": deepcopy(self.states) if self.copy else self.states,
                "reached_goal": [i["reached_goal"] for i in infos],
                }
    ...
    def _check_state_spaces(self):
        self._assert_is_running()
        for pipe in self.parent_pipes:
            pipe.send(("_check_state_space", self.single_state_space))
        same_spaces, successes = zip(*[pipe.recv() for pipe in self.parent_pipes])
        self._raise_if_errors(successes)
        if not all(same_spaces):
            raise RuntimeError(
                "Some environments have an state space "
                "different from `{0}`. In order to batch states, the "
                "state spaces from all environments must be "
                "equal.".format(self.single_state_space)
            )
...
def _worker(index, env_fn, pipe, parent_pipe, shared_memory_obs, shared_memory_states, error_queue):
    assert shared_memory_obs is None
    assert shared_memory_states is None
    env = env_fn()
    parent_pipe.close()
    try:
        while True:
            ...
            if command == "reset":
                ...
                pipe.send((observation, state, True))
            elif command == "step":
            ...
            elif command == "_check_state_space":
                pipe.send((data == env.state_space, True))
            else:
                raise RuntimeError(
...
def _worker_shared_memory(index, env_fn, pipe, parent_pipe, shared_memory_obs, shared_memory_states, error_queue):
    assert shared_memory_obs is not None
    assert shared_memory_states is not None
    env = env_fn()
    ...
    try:
        while True:
            ...
            if command == "reset":
                observation, state = env.reset()
                write_to_shared_memory(
                    index, observation, shared_memory_obs, observation_space
                )
                write_to_shared_memory(
                    index, state, shared_memory_states, state_space
                )
                ...
                write_to_shared_memory(
                    index, info["state"], shared_memory_states, state_space
                )
                ...
                pipe.send((data == state_space, True))
            else:
                raise RuntimeError(
                    "Received unknown command `{0}`. Must "
                    "be one of {`reset`, `step`, `seed`, `close`, "
