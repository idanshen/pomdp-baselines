...
class ModelFreeOnPolicy_Separate_RNN(nn.Module):
    """Recommended Architecture
    Recurrent Actor and Recurrent Critic with separate RNNs
    """
    ...
    @torch.no_grad()
    def get_initial_info(self):
        return self.actor.get_initial_info(), self.value.get_initial_info()
    ...
    def predict_value(
        ...
        value, current_internal_state = self.value.predict(
            prev_internal_state=prev_internal_state,
            prev_action=prev_action,
            reward=reward,
            obs=obs,
        ...
        return value, current_internal_state
    ...
    def forward(self, actions, rewards, observs, dones, masks):
        ...
        ### 1. Value loss
        v_pred, value_loss = self.algo.value_loss(
            markov_actor=self.Markov_Actor,
            markov_critic=self.Markov_Critic,
            actor=self.actor,
            actor_target=self.actor_target,
