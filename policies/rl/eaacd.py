...
class EAACD(RLAlgorithmBase):
        ...
        **kwargs
        ...
        if self.coefficient_tuning == "EIPO":
            aux_qf1_loss, aux_qf2_loss = self._critic_loss("aux",
                              ...
                              teacher_next_log_probs=teacher_next_log_probs,
                              **kwargs)
            loss_dict_q1["aux_loss"] = aux_qf1_loss
            loss_dict_q2["aux_loss"] = aux_qf2_loss
        ...
        **kwargs
        ...
        if self.split_q and key == "main":
            with torch.no_grad():
                ...
                min_next_q_env_target = (new_probs * min_next_q_env_target).sum(
                    dim=-1, keepdims=True
                )
            ...
            if markov_critic:
                ...
                q1_teacher_pred = q1_teacher_pred.gather(dim=-1, index=action)
                q2_teacher_pred = q2_teacher_pred.gather(dim=-1, index=action)
                q1_env_pred = q1_env_pred.gather(dim=-1, index=action)
                q2_env_pred = q2_env_pred.gather(dim=-1, index=action)
                ...
                q1_teacher_pred = q1_teacher_pred.gather(dim=-1, index=stored_actions)  # (T, B, A) -> (T, B, 1)
                q2_teacher_pred = q2_teacher_pred.gather(dim=-1, index=stored_actions)  # (T, B, A) -> (T, B, 1)
                q1_env_pred = q1_env_pred.gather(dim=-1, index=stored_actions)  # (T, B, A) -> (T, B, 1)
                q2_env_pred = q2_env_pred.gather(dim=-1, index=stored_actions)  # (T, B, A) -> (T, B, 1)
    ...
    def update_others(self, additional_outputs, markov_critic, markov_actor, critic, actor, observs, actions, rewards):
        ...
        if self.coefficient_tuning == "EIPO":
            ...
            self.log_coefficient = torch.clip(self.log_coefficient + self.coefficient_lr * objective_difference, -4, -0.7)
            self.coefficient = self.log_coefficient.exp().item()
