...
class PPO:
    ...
    @staticmethod
    def build_value(hidden_sizes, input_size=None, obs_dim=None, **kwargs):
        if obs_dim is not None:
            input_size = obs_dim
        vf = FlattenMlp(
    ...
    def value_loss(
        ...
        with torch.no_grad():
            if isinstance(actor.action_space, spaces.Discrete):
    ...
    def loss_over_batch(self, rollout_data) -> None:
        # Compute current clip range
        clip_range = self.clip_range
        # Optional: clip range for the value function
        if self.clip_range_vf is not None:
